...
*  state (which is constant) where the K_3 term dominates.
*
*  The configuration is n = (cos(theta), sin(theta), 0)
*  theta = (PI/2) - arcsin( 1 - (y + 1)/(height +1) )
*
* We check two things:
...
int main(int n, char* argv[])
{
	...
	configuration.initialState = LatticeConfig::K3_DOMINANT;
	...
	//set lattice beta value (assume k_3 >> k_1) - the code doesn't support k_1==0 so just set k_3 very large for now!
	configuration.beta = 1e22;
	...
	*/
	...
	analyticalFreeEnergy= (double) (configuration.beta)/( 2*(configuration.height +1)*(configuration.height +1) );
